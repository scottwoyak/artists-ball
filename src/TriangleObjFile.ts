...
export class TriangleObjFile extends TriangleBase {
   ...
   create(url: string, center: glVec3): Promise<void> {
      ...
      return fetch(url)
         .then(res => res.text())
         .then(res => {
            let triangles = this.createTriangles(res, center);
            super.store(triangles);
         });
   }

   private createTriangles(src: string, center: glVec3): Triangle[] {
      ...
      for (let i = 0; i < lines.length; i++) {
         ...
         if (line.startsWith('v ')) {
            ...
            if (tokens.length === 5) {
               ...
               triangles.push(new Triangle(vertices[i1].clone(), vertices[i2].clone(), vertices[i3].clone()));
               triangles.push(new Triangle(vertices[i1].clone(), vertices[i3].clone(), vertices[i4].clone()));
            }
            else {
               let i1 = parseInt(tokens[1].split('/')[0]) - 1;
               let i2 = parseInt(tokens[2].split('/')[0]) - 1;
               let i3 = parseInt(tokens[3].split('/')[0]) - 1;
               triangles.push(new Triangle(vertices[i1], vertices[i2], vertices[i3]));
            }
         }
      }
